                       ...
                       @endif
                        ...
                        <div class="col-md-5">
                            <div class="slider-text">
                                ...
                                <p><span>Offered by: {{!empty($user)?$user->name:''}}</span>
                                    <span> Post ID:{{$dontaion_post->post_no}}</span></p>
                                <span class="icon"><i class="fa fa-clock-o"></i>{{date('d-m-Y H:i:s', strtotime($dontaion_post->created_at) ) }}</span>
                                <!-- <span class="icon"><i class="fa fa-map-marker"></i><a href="#">{{ $city->name. ', '.$state->name.', '.$country->name }}</a></span> -->
                                <span class="icon"><i class="fa fa-map-marker"></i>{{ $dontaion_post->address  }}</span>
                                
                                <span class="icon"><i class="fa fa-suitcase "></i>{{ $user_type->name }} <strong></strong></span>

                                <!-- short-info -->
                                <div class="short-info">
                                    <p><strong>Condition: </strong>{{ $dontaion_post->condition == 1 ? "New" : "old"  }} </p>
                                ...
                                <div class="contact-with">
                                    <h4>Contact with </h4>
                                    <a href="tel:{{ !empty($user)?$user->contact:'' }}">
                                    ...
                                    <a href="mailto:{{ !empty($user)?$user->email:'' }}" class="btn"><i class="fa fa-envelope-square"></i>{{ !empty($user)?$user->email:'' }}</a>
                                </div><!-- contact-with -->
...
<script type="text/javascript">
    ...
    })(marker, createInfo('{{ $user_type->name }}','<a href="tel:{{ !empty($user)?$user->contact:'' }}">{{ !empty($user)?$user->contact:'' }}</a>')));
   ...
   var infoWindow1 = new google.maps.InfoWindow({
    ...
    })(marker1, createInfo('Donner/donee','<a href="tel:{{ !empty($user)?$dontaion_post->d_contact:'' }}">{{ !empty($dontaion_post)?$dontaion_post->d_contact:'' }}</a>')));
   ...
   google.maps.event.addListener(marker2, 'click', (function(mm1, tt1) {
    ...
    })(marker2, createInfo('Helper','<a href="tel:{{ !empty($dontaion_post)?$dontaion_post->helper_contact:'' }}">{{ !empty($dontaion_post)?$dontaion_post->helper_contact:'' }}</a>')));
