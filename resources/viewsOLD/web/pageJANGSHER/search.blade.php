...
Imtech.Pager = function() {
    ...
    var renderControls = function(container, currentPage, numPages) {
        var pagingControls = '<nav aria-label="..."><ul class="pagination pagination-lg">';
        for (var i = 1; i <= numPages; i++) {
            if (i != currentPage) {
                pagingControls += '<li class="page-item"><a class="page-link" href="#" onclick="pager.showPage(' + i + '); return false;">' + i + '</a></li>';
            } else {
                pagingControls += '<li class="page-item disabled"><a class="page-link">' + i + '</a></li>';
            }
        }
    ...
    function call_ajax(url,data){
        $.ajaxSetup({
            ...
            success: function(data){
                ...
                $('.ajax-loading').hide(); 
                var htmldata = '<div id="content">'+data+'</div><div id="pagingControls"></div>';
                $('.appendText').html(htmldata);
                
                
                    pager.paragraphsPerPage = 1; // set amount elements per page
                    pager.pagingContainer = $('#content'); // set of main container
                    pager.paragraphs = $('div.item-image-box', pager.pagingContainer); // set of required containers
                    pager.showPage(1);
